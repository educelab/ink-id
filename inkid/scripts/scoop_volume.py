...
def main():
    ...
    parser.add_argument('-i', '--input-volume-dir', required=True,
                        help='Input volume slices to be modified (scooped)')
    parser.add_argument('-p', '--projections-dir', required=True,
                        help='Slices with a projected mesh, below which will be scooped out')
    parser.add_argument('-o', '--output-volume-dir', required=True, help='Output directory for scooped slices')
    parser.add_argument('-f', '--fill', type=int, default=32768,
