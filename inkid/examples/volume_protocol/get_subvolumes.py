...
remote_host = (
    sys.argv[1] if (len(sys.argv) > 1 and len(sys.argv[1]) > 0) else "127.0.0.1"
)
remote_port = int(sys.argv[2]) if (len(sys.argv) > 2 and len(sys.argv[2]) > 0) else 8087

request = RequestArgs(
    volpkg="PHercParis Objet 59",
    volume="20190910132130",
    center_x=152.5,
    center_y=485.0,
    center_z=605.0,
    sampling_r_x=152.5,
    ...
    for z in range(args.extent_z):
        ...
        for y in range(args.extent_y):
            for x in range(args.extent_x):
                true_index = (
                    (z * args.extent_y * args.extent_x) + (y * args.extent_x) + x
                )
                slice_data.append(downsampled[true_index])
        image = Image.frombytes("L", (args.extent_x, args.extent_y), bytes(slice_data))
