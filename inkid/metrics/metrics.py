...
def positive_preds(pred, yb):
    tp, fp, _, _ = confusion(pred, yb)
    return tp + fp


def negative_preds(pred, yb):
    _, _, tn, fn = confusion(pred, yb)
    return tn + fn


def positive_labels(_, yb):
    return int(sum(yb))


def negative_labels(_, yb):
    return len(yb) - int(torch.count_nonzero(yb))
