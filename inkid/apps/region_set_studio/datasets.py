...
class Datasource:
    ...
    def fromPath(path: Path):
        ...
        try:
            ...
            raise DatasetError(f"Failed to open datasource: {path}") from os_error
        except JSONDecodeError as json_error:
            raise DatasetError(
                f"Failed to parse JSON for datasource: {path}"
            ) from json_error
        except KeyError as key_error:
            raise DatasetError(
                f"Expected element is missing: {key_error}"
            ) from key_error
...
class DatasetEditor(QWidget):
    ...
    def _add_items(self, new_items):
        ...
        for filename in new_items:
            if filename not in items:
                relative_path = os.path.relpath(filename, start=self._path.parents[0])
    ...
    def update_buttons(self, current: int):
        self.btn_del.setEnabled(current >= 0 and self.list_view.item(current) != None)
    ...
    def new_item(self, checked: bool = True):
        filename, filter_ = QFileDialog.getSaveFileName(
            self,
            "New Dataset or Datasource",
            dir=str(self._path.parents[0]),
            filter="Datasets (*.txt);;Datasources (*.json)",
        )
        if len(filename) < 1:
            return
        try:
    ...
    def add_item(self, checked: bool = False):
        filenames = QFileDialog.getOpenFileNames(
            self,
            "Find Datasets and Datasources",
            dir=str(self._path.parents[0]),
            filter="Datasets and Datasources (*.txt *.json)",
        )[0]
    ...
    def browse_file(self, checked: bool = False):
        if self.filter is None:
            ...
            filename = QFileDialog.getOpenFileName(
                self, "Browse File", self.dir, self.filter
            )[0]
        if len(filename) < 1:
            return
...
class RegionBoundsDialog(QDialog):
    def __init__(self, parent, datasource: Datasource, ghosts: list):
        ...
        scene.setSceneRect(0, 0, self._ppm_data["width"], self._ppm_data["height"])
        ...
        if existing_bounds is None:
            ...
            self._saved_value = [
                0,
                0,
                self._ppm_data["width"] - 1,
                self._ppm_data["height"] - 1,
            ]
        else:
        ...
        self.underlay_list.currentItemChanged.connect(self.underlay_selection_changed)
        for underlay in [
            self._datasource.getMask(True),
            self._datasource.getInkLabel(True),
            self._datasource.getRGBLabel(True),
        ]:
            if underlay is None:
                continue
    ...
    @Slot(QListWidgetItem, QListWidgetItem)
    def underlay_selection_changed(
        self, current: QListWidgetItem, previous: QListWidgetItem
    ):
        data = current.data(Qt.UserRole + 1)
    ...
    def action_apply(self, checked: bool = False):
        ...
        self._saved_value = [
            self.bounds_x1.value(),
            self.bounds_y1.value(),
            self.bounds_x2.value(),
            self.bounds_y2.value(),
        ]
        self._tainted = False
        self.btn_apply.setEnabled(False)
    ...
    def browse_underlays(self, checked: bool = False):
        filenames = QFileDialog.getOpenFileNames(
            self,
            "Browse Images",
            str(self._datasource.getPath().parents[0]),
            "Image Files (*.png *.tif *.tiff *.jpg *.jpeg)",
        )[0]
    ...
    def change_pos(self, value):
        ...
        self.bounding_box_gfx.change_pos(
            self.bounds_x1.value(),
            self.bounds_y1.value(),
            self.bounds_x2.value(),
            self.bounds_y2.value(),
...
class DatasourceEditor(QWidget):
    ...
    def __init__(self, path: Path, datasources_paths: list, parent: Optional[QObject]):
        ...
        self.ds_type.setCurrentIndex(1 if self._datasource.getType() == "region" else 0)
        self.ds_type.currentTextChanged.connect(self.update_type)

        self.ds_volume = FileBrowserWidget(
            self._datasource.getVolume(),
            str(self._datasource.getPath().parents[0]),
            None,
        )
        self.ds_volume.changed.connect(self.update_volume)
        self.ds_ppm = FileBrowserWidget(
            self._datasource.getPPM(),
            str(self._datasource.getPath().parents[0]),
            "PPM Files (*.ppm)",
        )
        self.ds_ppm.changed.connect(self.update_ppm)
        self.ds_mask = FileBrowserWidget(
            self._datasource.getMask(),
            str(self._datasource.getPath().parents[0]),
            "Image Files (*.png *.tiff *.tif)",
        )
        self.ds_mask.changed.connect(self.update_mask)
        self.ds_ink_label = FileBrowserWidget(
            self._datasource.getInkLabel(),
            str(self._datasource.getPath().parents[0]),
            "Image Files (*.png *.tiff *.tif)",
        )
        self.ds_ink_label.changed.connect(self.update_ink_label)
        self.ds_rgb_label = FileBrowserWidget(
            self._datasource.getRGBLabel(),
            str(self._datasource.getPath().parents[0]),
            "Image Files (*.png *.tiff *.tif)",
        )
        self.ds_rgb_label.changed.connect(self.update_rgb_label)
        self.ds_vct_label = FileBrowserWidget(
            self._datasource.getVCTLabel(),
            str(self._datasource.getPath().parents[0]),
            "Image Files (*.png *.tiff *.tif)",
        )
    ...
    def switch_files(self, filename: str):
        if self._tainted:
            save_yorn = QMessageBox.question(
                self,
                "Save changes?",
...
class DatasetModel(QStandardItemModel):
    ...
    def _load_dataset(
        self, parent_item: QStandardItem, path: Path, relative_to: Path = None
    ):
        ...
        try:
            with open(path) as f:
                for line in f.readlines():
                    self._load_dataset(
                        dataset_item, Path(line.strip()), relative_to=path.parents[0]
                    )
        except OSError as os_err:
            raise DatasetError(os_err) from os_err
