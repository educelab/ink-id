...
def buildModel(x, y, drop_rate, args):
    ...
    conv1 = slim.batch_norm(slim.convolution(x, args["neurons"][0], [3, 3, 3], stride=[1,1,2], padding='valid'))
    conv2 = slim.batch_norm(slim.convolution(conv1, args["neurons"][1], [3, 3, 3], stride=[1,1,2], padding='valid'))
    conv3 = slim.batch_norm(slim.convolution(conv2, args["neurons"][2], [3, 3, 3], stride=[1,1,2], padding='valid'))
    conv4 = slim.batch_norm(slim.convolution(conv3, args["neurons"][3], [3, 3, 3], stride=[1,1,2], padding='valid'))
